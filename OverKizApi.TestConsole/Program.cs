...
internal static class Program
   ...
   private static async Task Main ()
      ...
      // ── Credentials ──────────────────────────────────────────────────
		...
		var serverChoices = new Dictionary<int, Server>
			{
      ...
      if (isLocalMode)
         ...
         string localEndpoint = OverkizConst.LocalEndpoint(gatewayIp);
         ...
         if (saved is not null)
            {
            Console.Write ($"Username [{saved.Username}]: ");
            string input2 = Console.ReadLine ()?.Trim () ?? string.Empty;
         ...
         string password = (saved is not null && string.IsNullOrEmpty (typedPassword))
            ? saved.Password
            : typedPassword;
         ...
         client = new OverkizClient (username, password, server);
         }
      ...
      await using (client)
         ...
         try
            ...
            if (!isLocalMode)
               ...
               CredentialStore.Save (selectedServer, new SavedCredential { Username = client.Username, Password = client.Password });
            ...
            while (running)
               ...
               switch (input)
                  {
                  case "1": await RunAction (() => ShowSetupSummary (client)); break;
                  case "2": await RunAction (() => ListDevices (client)); break;
                  case "3": await RunAction (() => ListGateways (client)); break;
                  case "4": await RunAction (() => ListScenarios (client)); break;
                  case "5": await RunAction (() => GetDeviceStates (client)); break;
                  case "6": await RunAction (() => RefreshStates (client)); break;
                  case "7": await RunAction (() => ShowExecutionHistory (client)); break;
                  case "8": await RunAction (() => ShowCurrentExecutions (client)); break;
                  case "9": await RunAction (() => ListLocalTokens (client)); break;
                  case "a":
                  case "A": await RunAction (() => ControlDevice (client)); break;
                  case "e":
                  case "E": await RunAction (() => WatchEvents (client)); break;
   ...
   private static async Task ControlDevice (OverkizClient client)
      ...
      for (int i = 0; i < chosen.NParams; i++)
		...
		Command command = new () { Name = chosen.CommandName!, Parameters = parameters.Count > 0 ? (IReadOnlyList<object?>)parameters : null };
	...
	private static async Task WatchEvents (OverkizClient client)
		...
		try
			...
			while (!cts.Token.IsCancellationRequested)
				...
				var (events, rawJson) = await client.FetchEventsRaw ();
			...
			try { await client.UnregisterEventListener (); }
			catch (Exception ex) { Console.WriteLine ($"[WARN] Unregister failed: {ex.Message}"); }
	...
	private static void PrintEvent (EventObject ev)
		...
		if (ev.DeviceStates.Count > 0)
			...
			if (ev.SubType.HasValue) extras.Append ($"  subType={ev.SubType}");
			if (ev.NewState.HasValue) extras.Append ($"  newState={ev.NewState}");
			if (ev.OldState.HasValue) extras.Append ($"  oldState={ev.OldState}");
			if (ev.ExecId    != null) extras.Append ($"  execId={ev.ExecId}");
